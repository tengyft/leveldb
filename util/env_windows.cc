...
/// NOMINMAX 宏可以让 windows 不定义 min/max 宏，从而直接使用 stl 定义的 min/max 函数
...
std::string GetWindowsErrorMessage(DWORD error_code) {
    ...
    size_t error_text_size =
        ::FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_IGNORE_INSERTS, nullptr, error_code,
                         MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), reinterpret_cast<char*>(&error_text), 0, nullptr);
    if (!error_text) {
        return message;
    }
...
public:
    WindowsSequentialFile(std::string filename, ScopedHandle handle) : handle_(std::move(handle)), filename_(std::move(filename)) {}
...
public:
    WindowsRandomAccessFile(std::string filename, ScopedHandle handle) : handle_(std::move(handle)), filename_(std::move(filename)) {}
...
public:
    WindowsWritableFile(std::string filename, ScopedHandle handle) : pos_(0), handle_(std::move(handle)), filename_(std::move(filename)) {}
...
public:
    WindowsFileLock(ScopedHandle handle, std::string filename) : handle_(std::move(handle)), filename_(std::move(filename)) {}
...
public:
    ...
    Status NewRandomAccessFile(const std::string& filename, RandomAccessFile** result) override {
        *result                     = nullptr;
        ...
        if (mapping.is_valid()) {
            ...
            if (mmap_base) {
                *result = new WindowsMmapReadableFile(filename, reinterpret_cast<char*>(mmap_base), static_cast<size_t>(file_size.QuadPart),
                                                      &mmap_limiter_);
                return Status::OK();
            }
        }
    ...
    bool FileExists(const std::string& filename) override { return GetFileAttributesA(filename.c_str()) != INVALID_FILE_ATTRIBUTES; }
    ...
    Status GetChildren(const std::string& directory_path, std::vector<std::string>* result) override {
        ...
        do {
            ...
            if (!_splitpath_s(find_data.cFileName, nullptr, 0, nullptr, 0, base_name, ARRAYSIZE(base_name), ext, ARRAYSIZE(ext))) {
    ...
    Status RenameFile(const std::string& from, const std::string& to) override {
        ...
        if (::ReplaceFileA(to.c_str(), from.c_str(), /*lpBackupFileName=*/nullptr, REPLACEFILE_IGNORE_MERGE_ERRORS,
    ...
    Status LockFile(const std::string& filename, FileLock** lock) override {
        *lock = nullptr;
        Status       result;
        ScopedHandle handle = ::CreateFileA(filename.c_str(), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ,
...
WindowsEnv::WindowsEnv() : background_work_cv_(&background_work_mutex_), started_background_thread_(false), mmap_limiter_(MaxMmaps()) {}

void WindowsEnv::Schedule(void (*background_work_function)(void* background_work_arg), void* background_work_arg) {
